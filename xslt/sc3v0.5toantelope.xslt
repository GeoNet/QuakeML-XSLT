...
<xsl:variable name="originTime" select="time/value"></xsl:variable>

            <xsl:value-of
                select="string('#'), geonet:epoch(time/value), year-from-dateTime(time/value), month-from-dateTime(time/value), day-from-dateTime(time/value), hours-from-dateTime(time/value), minutes-from-dateTime(time/value), seconds-from-dateTime(time/value), longitude/value, latitude/value, depth/value"/>
    ...
    <xsl:template match="magnitude">
        ...
        <xsl:if test="@publicID=$preferredMagnitudeID">
            <xsl:value-of select="string(''), magnitude/value"/>
        </xsl:if>
    </xsl:template>
    
    <xsl:template match="EventParameters/pick">
        ...
        <xsl:if test="@publicID=$pickID">
            <xsl:value-of
                select="waveformID/@networkCode, waveformID/@stationCode, waveformID/@channelCode, waveformID/@locationCode, $phaseName, geonet:epoch(time/value), string(''), geonet:pickTimeDiff($originTime, time/value), $newline"
            />
        </xsl:if>
    </xsl:template>
    ...
    <xsl:function name="geonet:pickTimeDiff">
        ...
        <xsl:value-of separator=":" select="format-number(hours-from-duration($diff),'00'), format-number(minutes-from-duration($diff),'00'), format-number(seconds-from-duration($diff), '00.000000')"></xsl:value-of>
    </xsl:function>
